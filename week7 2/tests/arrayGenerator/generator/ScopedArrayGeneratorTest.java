...
abstract class ScopedArrayGeneratorTest<T> extends ArrayGeneratorTest<T> {
    ...
    /**
     *Inspects a generated array to make sure it contains all expected values.
     *
     * @param scope the arrayGenerator.scope to be tested.
     */
    void testCoversRange(Scope<T> scope) {
        ...
        Set<T> viewed = new HashSet<T>();
        for (T entry: array) {
            if (!viewed.contains(entry)) {
                if (!scope.contains(entry)) {
                    fail("Array entry is not within expected bounds.");
                } else {
                    viewed.add(entry);
                    if (viewed.size() == scope.size()) {
                        break;
                    }
                }
            }
        ...
        if (viewed.size() < scope.size()) {
            fail("Array did not contain all expected values.\n"
                 ...
                 + "The scope is " + scope + "\n"
                 + "Saw " + viewed +"\n"
                 + "IMPORTANT!: There is a " + ACCEPTABLE_FALSE_FAIL + " chance that this is a false fail.  See the method documentation for details.");
        }
    }
}
/**
